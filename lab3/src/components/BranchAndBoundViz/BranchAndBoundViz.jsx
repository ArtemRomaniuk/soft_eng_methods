...
export default function BranchAndBoundViz({ weights, values, capacity }) {
  ...
  useEffect(() => {
    ...
    const links = g
      ...
      .attr("fill", "none")
      .attr("stroke", (d) =>
        d.target.data.isBest ? "#00ff80" : "var(--border-light)",
      )
    ...
    nodes
      ...
      .attr("r", 14)
      .attr("fill", (d) =>
        d.data.isBest ? "rgba(0, 255, 128, 0.2)" : "var(--bg-tertiary)",
      )
      .attr("stroke", (d) =>
        d.data.isBest ? "#00ff80" : "var(--border-accent)",
      )
    ...
    nodes
      ...
      .text((d) =>
        d.data.type === "root"
          ? "Root"
          : `${d.data.type === "take" ? "T" : "S"} I:${d.data.itemInfo.id + 1}`,
      );
    ...
    nodes
      ...
      .style("font-size", "7px")
      .text((d) =>
        d.data.itemInfo ? `w:${d.data.itemInfo.w} v:${d.data.itemInfo.v}` : "",
      );
    ...
    nodes
      ...
      .style("font-size", "7px")
      .attr("fill", (d) =>
        d.data.isBest ? "#00ff80" : "var(--text-secondary)",
      )
    ...
    nodes
      .filter((d) => d.data.isPruned && !d.data.isBest)
      .append("text")
      .attr("x", 12)
      .attr("y", -12)
      .attr("fill", "#ff4d4d")
      ...
      <Controls>
        <ResultsInfo>
          <ResultItem className="max-value">
            <strong>B&B Result:</strong> {maxValue}
          </ResultItem>
